...
def send(adid):
    ...
    description, title, image, status, category, area, replymail, username, password, acountID, ip, port = cursor.fetchone()
    ...
    s.sendall("POST / HTTP/1.1 \n Content-Type: application/x-www-form-urlencoded \n\n" + \
              render(replymail,title,description,username,password,acountID,category,area,image))
    print s.recv(4096)
